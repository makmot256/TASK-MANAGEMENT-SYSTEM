...
export default function ReviewDetail() {
  ...
  const [aiAction, setAiAction] = useState<'none' | 'accepted' | 'overridden'>('none');
  ...
  // Poll while the AI evaluation is still pending (it runs async right after submission).
  useEffect(() => {
    if (data?.ai_evaluation?.status !== 'pending') return;
    const t = setInterval(load, 3000);
    return () => clearInterval(t);
  }, [data?.ai_evaluation?.status]);

  const acceptAiScore = () => {
    if (!data?.ai_evaluation) return;
    setQuality(Math.round(Number(data.ai_evaluation.quality_score)));
  ...
  const recheckAi = async () => {
    setRechecking(true);
    try {
      await api.post(`/submissions/${id}/ai-evaluate`);
      toast('AI evaluation re-run.', 'success');
      load();
    } catch (err) { toast(errMsg(err), 'error'); } finally { setRechecking(false); }
  };
  ...
  const assess = async (opts: any) => {
    ...
    setBusy(true);
    try {
      await api.post(`/submissions/${id}/assess`, { quality_score: quality, ai_action: aiAction, ...opts });
        ...
        <div style={{ display: 'grid', gap: 18, alignSelf: 'start' }}>
          <div className="card card-pad">
            <div className="row between" style={{ marginBottom: 10 }}>
              <h3 className="card-title" style={{ margin: 0 }}>AI evaluation</h3>
              <span className="tiny">Supervisor-only</span>
            </div>
            {!data.ai_evaluation || data.ai_evaluation.status === 'pending' ? (
              <p className="muted" style={{ fontSize: 13, margin: 0 }}>
                <Spinner /> Scoring the report for grammar and quality…
              </p>
            ) : data.ai_evaluation.status === 'failed' ? (
              <div>
                <p className="tiny" style={{ color: 'var(--danger, #c0392b)', marginBottom: 8 }}>
                  AI check failed: {data.ai_evaluation.error_message || 'unknown error'}
                </p>
                <button className="btn btn-ghost btn-sm" onClick={recheckAi} disabled={rechecking}>
                  {rechecking ? <Spinner /> : 'Retry AI check'}
                </button>
              </div>
            ) : (
              <>
                <div className="row" style={{ gap: 20, marginBottom: 10 }}>
                  <div>
                    <div className="tiny">Grammar</div>
                    <div style={{ fontWeight: 700, fontSize: 20 }}>{Number(data.ai_evaluation.grammar_score).toFixed(1)}/5</div>
                  </div>
                  <div>
                    <div className="tiny">Quality</div>
                    <div style={{ fontWeight: 700, fontSize: 20 }}>{Number(data.ai_evaluation.quality_score).toFixed(1)}/5</div>
                  </div>
                </div>
                <div className="row" style={{ gap: 8 }}>
                  <button className="btn btn-ghost btn-sm" onClick={acceptAiScore}>Use as quality score</button>
                  <button className="btn btn-ghost btn-sm" onClick={recheckAi} disabled={rechecking}>
                    {rechecking ? <Spinner /> : 'Re-run'}
                  </button>
                </div>
                {aiAction === 'accepted' && <p className="tiny" style={{ marginTop: 8 }}>Using AI score — adjust the stars below to override manually.</p>}
              </>
            )}
          </div>
          ...
          <div className="field"><label className="label">Quality score (0-5)</label><Stars value={quality} onPick={(n) => { setQuality(n); setAiAction('overridden'); }} /></div>
