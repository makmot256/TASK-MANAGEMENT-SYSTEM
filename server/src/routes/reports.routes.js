...
import { runAiEvaluation, getAiEvaluation, markAiEvaluationAction } from '../services/ai-evaluation.service.js';
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Fire-and-forget: score grammar/quality via the ML service. Never blocks
    // or fails the submission response — supervisor just sees "pending" until done.
    runAiEvaluation(submissionId).catch((err) => console.error('[ai-evaluation] unexpected error:', err));
  ...
  asyncHandler(async (req, res) => {
    ...
    const [rows] = await pool.query(
      `SELECT s.id, s.kind, s.is_late, s.submitted_at, s.revision_requested_at, s.revision_of,
              ...
              ae.grammar_score AS ai_grammar_score, ae.quality_score AS ai_quality_score, ae.status AS ai_status
       FROM submissions s
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // and the AI grammar/quality evaluation. Members never see either.
    let peer_reviewers = [];
    let ai_evaluation = null;
    if (req.user.role === 'supervisor' || req.user.role === 'admin') {
      peer_reviewers = await getPeerReviewersForSubmission(req.params.id);
      ai_evaluation = await getAiEvaluation(req.params.id);
    }
...
// POST /api/submissions/:id/ai-evaluate  (re-run AI scoring, e.g. after a failure)
router.post(
  '/:id/ai-evaluate',
  requireRole('supervisor', 'admin'),
  asyncHandler(async (req, res) => {
    await runAiEvaluation(req.params.id);
    const evaluation = await getAiEvaluation(req.params.id);
    res.json({ ai_evaluation: evaluation });
  })
);
...
router.post(
  ...
  requireRole('supervisor', 'admin'),
  asyncHandler(async (req, res) => {
    const { quality_score, request_revision, mark_completed, ai_action } = req.body;
    const quality = Number(quality_score);
    if (!(quality >= 0 && quality <= 5)) throw new HttpError(400, 'Quality score must be between 0 and 5.');

    // ai_action: 'accepted' when supervisor used the AI score as-is,
    // 'overridden' when they typed in their own score instead. Optional.
    if (ai_action === 'accepted' || ai_action === 'overridden') {
      await markAiEvaluationAction(req.params.id, ai_action);
    }
