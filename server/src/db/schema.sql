...
-- AI grammar/quality evaluation, generated automatically when a report is
-- submitted. Visible to supervisors/admins only — never surfaced to the
-- member who submitted the report (enforced at the API layer).
CREATE TABLE IF NOT EXISTS ai_evaluations (
  id             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  submission_id  BIGINT UNSIGNED NOT NULL,
  grammar_score  DECIMAL(3,2)    NULL,   -- 0.00 - 5.00
  quality_score  DECIMAL(3,2)    NULL,   -- 0.00 - 5.00
  model_name     VARCHAR(60)     NULL,
  status         ENUM('pending','done','failed') NOT NULL DEFAULT 'pending',
  error_message  VARCHAR(500)    NULL,
  supervisor_action ENUM('none','accepted','overridden') NOT NULL DEFAULT 'none',
  created_at     DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at     DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (id),
  UNIQUE KEY uq_ai_eval_submission (submission_id),
  CONSTRAINT fk_ai_eval_submission FOREIGN KEY (submission_id) REFERENCES submissions (id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
